...
export function Hero() {
  ...
  useEffect(() => {
    ...
    const animarTexto = async () => {
      ...
      textRef.current.textContent = textos[i].text;

      const { words } = splitText(textRef.current, { words: { wrap: "clip" } });

      words.forEach((w: HTMLElement) => {
        w.style.backgroundImage = `linear-gradient(to right, ${textos[i].gradient[0]}, ${textos[i].gradient[1]})`;
        w.style.backgroundSize = "200% 200%";
        w.style.webkitBackgroundClip = "text";
        w.style.color = "transparent";
        w.classList.add("gradient-animate");
      });
      ...
      indexRef.current = (indexRef.current + 1) % textos.length;
